...
# Behält dabei die ursprüngliche Array-Struktur bei

# Pfad zu den Dateien
$backupJsonPath = ".\public\tests_backup_einheiten_2025-04-22-19-31-43.json"
...
function Convert-EinheitToId {
    try {
        # Dateien einlesen
        Write-ColoredOutput "Lese Backup tests.json..." "Cyan"
        $testsJson = Get-Content -Path $backupJsonPath -Raw -Encoding UTF8
        ...
        for ($i = 0; $i -lt $totalTests; $i++) {
            ...
            if ($test.PSObject.Properties.Name -contains "einheit" -and $test.einheit -ne $null) {
                ...
                if ($einheitenMapping.ContainsKey($einheitWert)) {
                    ...
                    Write-Host "Nicht gefundene Einheit in Test ${i}: ${einheitWert}" -ForegroundColor Yellow
                }
        ...
        Set-Content -Path $outputJsonPath -Value $modifiedTestsJson -Encoding UTF8
        
        Write-ColoredOutput "`nKonvertierung abgeschlossen! $modifiedTests von $totalTests Tests wurden aktualisiert." "Green"
        Write-ColoredOutput "Original-Struktur: Array wurde beibehalten" "Green"
