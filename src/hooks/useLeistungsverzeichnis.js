 ...
 */
export default function useLeistungsverzeichnis() {  const [searchQuery, setSearchQuery] = useState('');
  const [selectedCategory, setSelectedCategory] = useState(null);
  const [selectedMaterial, setSelectedMaterial] = useState(null);
  
  // Debounce-Mechanismus für API-Anfragen
  const debounceTimerRef = useRef(null);
  const [shouldLoadTests, setShouldLoadTests] = useState(false);
  
  // Services laden
  const testsService = useTestsService();
  const profileService = useProfileService();
  const materialService = useMaterialService();
  const einheitenService = useEinheitenService();  // Lädt Tests basierend auf aktuellen Filtern - einfach aber effektiv
  const loadFilteredTests = useCallback(() => {
    console.log('useLeistungsverzeichnis: loadFilteredTests wird aufgerufen');
    const params = { search: searchQuery };
    testsService.loadTests(params);
  }, [searchQuery, testsService]);
  
  // Initialer Ladevorgang - einmal beim Starten
  useEffect(() => {
    // Sicherstellen, dass wir Tests laden, falls sie noch nicht vorhanden sind
    if (testsService && testsService.tests.length === 0) {
      console.log('useLeistungsverzeichnis: Initialer Ladevorgang');
      loadFilteredTests();
    }
  }, [testsService, loadFilteredTests]);
  
  // Suchanfrage aktualisieren
  const updateSearch = useCallback((query) => {
    console.log('useLeistungsverzeichnis: updateSearch mit Query:', query);
  ...
  const updateCategoryFilter = useCallback((category) => {
    ...
    // Der useEffect übernimmt das Laden
  }, []);
