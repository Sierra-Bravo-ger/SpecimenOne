 ...
 */
export const useProfileService = () => {  const [profiles, setProfiles] = useState([]);
  ...
  const loadProfiles = async () => {
    ...
    try {
      ...
      console.log('ProfileService: API-Antwort erhalten:', response);
      
      if (response && response.profile) {
        // Stellen Sie sicher, dass profile ein Array ist
        const profileData = Array.isArray(response.profile) ? response.profile : [];
        setProfiles(profileData);
        console.log('ProfileService: Profile geladen:', profileData.length, 'Erstes Profil:', profileData[0]);
        
        // Kategorien extrahieren, wenn noch nicht geladen
        if (categories.length === 0) {
          const uniqueCategories = [...new Set(profileData.map(profile => profile.kategorie))].filter(Boolean);
          setCategories(['Alle', ...uniqueCategories.sort()]);
        }
      } else {
        console.warn('ProfileService: Unerwartetes Antwortformat:', response);
        setProfiles([]);
      }
    } catch (err) {
  ...
  const getTestsFromProfile = (profileId) => {
    ...
    } else if (Array.isArray(profile.tests) && profile.tests.length > 0 && typeof profile.tests[0] === 'string' && profile.tests[0].includes('|')) {
      // Handle den Fall, dass tests ein Array mit einem pipe-getrennten String ist
      return profile.tests[0].split('|');
    }
